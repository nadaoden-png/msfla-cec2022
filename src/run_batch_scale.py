...
from benchmark_functions import evaluate, get_bounds
import algorithms as alg
import results_store_scale as store
...
def run_one(dim, func_name, algo_name, seed, max_fe):
    bounds = get_bounds(func_name)
    func = lambda x, fn=func_name: evaluate(fn, x)
...
def main():
    ...
    func_list = args.funcs.split(",")
    algo_list = args.algos.split(",")
    max_fe = 10000 * args.dim

    db = store.load_db()
    t_start = time.time()
    n_done_this_session = 0

    print(f"เริ่มรัน: D={args.dim}, MaxFE={max_fe}, ฟังก์ชัน={func_list}, "
          f"อัลกอริทึม={algo_list}, เวลาที่ให้={args.time_budget}s\n")
    ...
    while True:
        ...
        for func_name in func_list:
            for algo_name in algo_list:
                n = store.count_runs(db, args.dim, func_name, algo_name)
                if n < TARGET_RUNS:
                    job = (func_name, algo_name, n)
                    break
            if job:
                break
        ...
        func_name, algo_name, n_existing = job
        seed = abs(10000 * hash(algo_name + func_name) % 100000 + n_existing) % (2**31)

        t0 = time.time()
        best_f, n_evals, hist = run_one(args.dim, func_name, algo_name, seed, max_fe)
        t1 = time.time()
        round_times.append(t1 - t0)

        store.add_run(db, args.dim, func_name, algo_name, seed, best_f, n_evals, hist)
        store.save_db(db)

        n_done_this_session += 1
        print(f"  [{n_done_this_session}] D={args.dim} {func_name:10s} {algo_name:6s} "
              f"รอบที่ {n_existing+1}/{TARGET_RUNS}  best={best_f:.4e}  "
              f"evals={n_evals}  ({t1-t0:.1f}s)  "
              f"[เวลารวม {time.time()-t_start:.0f}/{args.time_budget:.0f}s]")
    ...
    print(f"\nสถานะรวมล่าสุด: {done}/{needed} รอบ")
    if lines:
